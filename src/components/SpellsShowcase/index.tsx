    ...
    {spellsList.map((item) => (
      <S.ShowcaseContent key={item.id}>
        ...
        {item.damageResume && item.damageResume.length > 0 && (
          <S.SpellDamage>
            {" "}
            <S.InforHover title="Esses dados resumem o dano nos níves 1, 5, 11 e 17">🛈</S.InforHover> Dano:{" "}
            {item.damageResume[0]}, {item.damageResume[1]}, {item.damageResume[2]}, {item.damageResume[3]} de dano de{" "}
            {item.damageType}
          </S.SpellDamage>
        )}
        ...
        <S.SpellComponents>
          {" "}
          <strong>Componentes:</strong> {item.components[0]}, {item.components[1]}, {item.components[2]}{" "}
        </S.SpellComponents>
        <S.SpellCastingTime>
          {" "}
